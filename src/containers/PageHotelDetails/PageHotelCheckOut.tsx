...
import { FaCalendar, FaCalendarAlt, FaUserAlt } from "react-icons/fa";
...
const CheckOutPage: FC<CheckOutPageProps> = ({ className = "" }) => {
  ...
  const { hotelInfo, isFetching } = useSelector(({ booking }: any) => booking)
  const [buttonDisable, setButtonDisable] = useState(false)
  const [hotelData, setHotelData]: any = useState(null)
  const [roomData, setRoomData]: any = useState(null)
  const [validateCode, setValidateCode]: any = useState(null)
  const [loading, setLoading] = useState(false)
  const [termsAccepted, setTermsAccepted] = useState(false)

  const getValidateData = (code: any) => {
    setLoading(true)
    Service.post({
      url: '/payment/pull/user/hotel',
      body: JSON.stringify({ code: code })
    }).then(response => {
      setLoading(false)
      if (response.status === 'success' && response.data) {
        setHotelData(response.data.hotelInfo)
        setRoomData(response.data.hotelInfo.rooms)
      } else {
        setHotelData([])
        setRoomData(null)
      }
  ...
  useEffect(()=> {
    ...
    if (params.get('validateId') && params.get('validateId') !== '') {
      getValidateData(params.get('validateId'))
      setValidateCode(params.get('validateId'))
    }
  ...
  const handleProceed = () => {
    ...
    Service.post({ url: `/payment/init/user/hotel`, body: JSON.stringify({ code: validateCode }) })
      .then(response => {
        ...
        if (response && response.status === 'success') {
          if (response.data === false) {
            ...
            eventTrack('Payment', 'Hotel', `${hotelData && hotelData.name}`)
      ...
      <div className="w-full flex flex-col sm:rounded-2xl sm:border border-neutral-200 dark:border-neutral-700 space-y-6 sm:space-y-8 px-0 sm:p-6 xl:p-8">
        <div className="flex flex-col space-y-4">
          ...
          <div className="flex justify-between text-neutral-6000 dark:text-neutral-300">
            ...
            <div><span className='currency-font'>{hotelData && hotelData.rate && getCurrency(hotelData.rate.baseFare.currency)}</span>{hotelData && hotelData.rate && amountSeparator(hotelData.rate.baseFare.amount)}</div>
          </div>
          <div className="flex justify-between text-neutral-6000 dark:text-neutral-300">
            <span>Discount</span>
            <div><span className='currency-font'>{hotelData && hotelData.rate && getCurrency(hotelData.rate.discount.currency)}</span>{hotelData && hotelData.rate && hotelData.rate.discount.amount}</div>
          </div>
          <div className="flex justify-between text-neutral-6000 dark:text-neutral-300">
            <span>Taxes & Service fees</span>
            <div><span className='currency-font'>{hotelData && hotelData.rate && getCurrency(hotelData.rate.tax.currency)}</span>{hotelData && hotelData.rate && hotelData.rate.tax.amount}</div>
          </div>

          <div className="border-b border-neutral-200 dark:border-neutral-700"></div>
          <div className="flex justify-between font-semibold">
            <span>Total Amount</span>
            <div><span className='currency-font'>{hotelData && hotelData.rate && getCurrency(hotelData.rate.totalFare.currency)}</span>{hotelData && hotelData.rate && amountSeparator(hotelData.rate.totalFare.amount)}</div>
          </div>
        </div>
    ...
    return (
      <div className="w-full flex flex-col sm:rounded-2xl sm:border border-neutral-200 dark:border-neutral-700 space-y-8 px-0 sm:p-6 xl:p-8">
        ...
        <div>
          <div className="flex flex-col sm:flex-row sm:items-center">
            ...
            <div className="py-5 sm:px-5 space-y-3">
              <div>
                ...
                <div className="text-sm text-neutral-500 dark:text-neutral-400 line-clamp-1 mt-2 block">
                  <i className="las la-map-marker-alt"></i>
                  <span className="ml-1"> {hotelData && hotelData.locationName}</span>
                </div>
              </div>
            ...
            <div className="flex-1 p-5 flex justify-between space-x-5">
              <div className="flex flex-col">
                ...
                <span className="mt-1.5 text-lg font-semibold">
                  {hotelData && moment(hotelData.checkIn).format('ll')}
                </span>
              </div>
              <FaCalendarAlt className="w-6 h-6 text-neutral-300 dark:text-neutral-6000" />
            </div>
            <div className="flex-1 p-5 flex justify-between space-x-5">
              <div className="flex flex-col">
                <span className="text-sm text-neutral-400">CHECK OUT</span>
                <span className="mt-1.5 text-lg font-semibold">
                  {hotelData && moment(hotelData.checkOut).format('ll')}
                </span>
              </div>
              <FaCalendarAlt className="w-6 h-6 text-neutral-300 dark:text-neutral-6000" />
            </div>
            {(roomData && roomData.length > 0 && roomData[0].rates && roomData[0].rates.length > 0) &&
              <div className="flex-1 p-5 flex justify-between space-x-5">
                <div className="flex flex-col">
                  <span className="text-sm text-neutral-400">GUESTS</span>
                  <span className="mt-1.5 text-lg font-semibold">{Number(roomData[0].rates[0].adults) + Number(roomData[0].rates[0].children)} Guests</span>
                </div>
                <FaUserAlt className="w-6 h-6 text-neutral-300 dark:text-neutral-6000" />
              </div>
            }
          </div>
          <div className="mt-6 border border-neutral-200 dark:border-neutral-700 rounded-3xl flex flex-col sm:flex-row divide-y sm:divide-x sm:divide-y-0 divide-neutral-200 dark:divide-neutral-700">
            <div className="flex flex-col p-5 sm:px-5 space-y-3">
              <h3 className="text-lg sm:text-lg lg:text-lg mt-0 font-semibold">
                {roomData && roomData.length > 0 && roomData[0].name}
              </h3>
              {(roomData && roomData.length > 0 && roomData[0].rates && roomData[0].rates.length > 0) &&
              <>
                <div>
                    <><span className="text-sm text-neutral-400">{`${roomData[0].rates[0].adults} Adults`}</span> <span className="text-sm text-neutral-200">&nbsp; | &nbsp; </span> <span className="text-sm text-neutral-400">{`${roomData[0].rates[0].rooms} Rooms`}</span> </>
                </div>
                <div key={21} className="pt-5 flex items-start space-x-3">
                  <i className="las la-check-circle text-2xl"></i>
                  <span className="text-sm">{roomData[0].rates[0].board}</span>
                </div>
              </>
              }
            </div>
          ...
          {(roomData && roomData.length > 0 && roomData[0].rates && roomData[0].rates.length > 0 && roomData[0].rates[0].refundable && roomData[0].rates[0].refundable.flag === true) &&
            <div className="flex flex-col mt-6 p-5 sm:px-5 space-y-3">
              <h3 className="text-lg sm:text-lg lg:text-lg mt-0 font-semibold">
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 gap-8 md:gap-5">
                <div key={21} className="pt-5 flex flex-col justify-start">
                  <p className="text-green"><strong>Free Cancellation</strong> until {moment(roomData[0].rates[0].refundable.rule.from).format('LT')} on {moment(roomData[0].rates[0].refundable.rule.from).format('ll')}</p>
                  <p className="text-neutral-6000 dark:text-neutral-300 mt-2">If you cancel booking after the date then cancellation charges will be <span className='currency-font'>৳</span><span className="text-sm font-semibold">{amountSeparator(roomData[0].rates[0].refundable.rule.amount)}</span></p>
                </div>
              </div>
            </div>
          }
  ...
  const renderSection2 = () => {
    return (
      <>
        {(roomData && roomData.length > 0 && roomData[0].rates && roomData[0].rates.length > 0 && roomData[0].rates[0].comments) &&
        <div className="listingSection__wrap p-5 bg-white dark:bg-neutral-900 mt-6">
          <div>
          ...
          <div className="text-sm text-neutral-700 dark:text-neutral-300 imp-info">
            <div key={1} className="flex flex-col items-start">
              {getImpInfo(roomData[0].rates[0].comments)}
            </div>
          </div>
        </div>
        }
