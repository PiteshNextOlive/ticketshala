...
const CardCategoryBox1 = ({ taxonomy }: any) => {
  ...
  const searchByPopular = (item: any) => {
    ...
    history.push(`/flights/search?${encodeURIComponent(`origin=${item.origin}&destination=${item.destination}&tripType=return&departDate=${moment().add(7, "days").format('YYYY-MM-DD')}&returnDate=${moment().add(10, "days").format('YYYY-MM-DD')}&cabinClass=Y&country=${aCounrty}&adults=1&child=0&infant=0&v=1.0`)}`)
  }
    ...
    >
      ...
      <div className="ml-4 flex-grow overflow-hidden">
        <h2 className="text-base font-medium">
          <span className="line-clamp-1">{getAirportCity(taxonomy.origin)} - {getAirportCity(taxonomy.destination)}</span>
        </h2>
      </div>
    </div>
  );
