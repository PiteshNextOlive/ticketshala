            ...
            <div className={`ml-6 sm:ml-0 sm:w-40`}>
              ...
              <div className="mt-1.5 flex items-start sm:items-start justify-start text-sm text-neutral-500 dark:text-neutral-400">
                <span className='text-xs truncate'>
                  {segments.map((items: any, x: any) => (
                    <>{`${items.airlineCode} ${items.flightNo}`}{(x !== segments.length - 1) && <>, </>}</>
                  ))}
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-col flex-grow">
            <div className="space-y-5 p-5">
              ...
              <div className="flex flex-row block sm:hidden md:hidden lg-hidden relative justify-start segments sm:flex-row items-start text-center sm:text-center">
                {renderMultiAirlines(col, col.options[0].flightSegment, 'm')}
              </div>
              ...
              <div className="flex items-center justify-between xl:justify-start sm:space-x-8 xl:space-x-12 text-sm text-neutral-700 dark:text-neutral-300">
                <div className="hidden sm:block flex flex-col md:flex md:flex-row relative justify-between segments sm:flex-row items-center text-center sm:text-left">
                  {renderMultiAirlines(col, col.options[0].flightSegment, 'd')}
                </div>
                <div className="flex flex-col sm:flex-row items-end space-y-3 sm:space-y-0 sm:text-right sm:space-x-3 ">
                  <div>
                    ...
                    <span className="block text-lg font-medium">
                      {moment(col.options[0].flightSegment[0].depTime).format('HH:mm')}
                    </span>
                    <div className="mt-1.5 text-xs text-neutral-500 dark:text-neutral-400">
                      {moment(col.options[0].flightSegment[0].depTime).format('ll')}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 text-center sm:text-left sm:space-x-3 ">
                  <div>
                    ...
                    <span className="block text-lg font-medium">
                      {moment(col.options[0].flightSegment[col.options[0].flightSegment.length - 1].arrTime).format('HH:mm')}
                    </span>
                    <span className="mt-1.5 w-20 text-xs text-neutral-500 dark:text-neutral-400">
                      {moment(col.options[0].flightSegment[col.options[0].flightSegment.length - 1].depTime).format('ll')}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-5 p-5">
              ...
              <div className="flex flex-row block sm:hidden md:hidden lg-hidden relative justify-start segments sm:flex-row items-start sm:space-y-3 sm:space-y-0 text-center sm:text-center sm:space-x-3 ">
                {col.options[1] && renderMultiAirlines(col, col.options[1].flightSegment, 'm')}
              </div>

              {/* 6 */}
              <div className="flex items-center justify-between xl:justify-start xl:space-x-12 text-sm text-neutral-700 dark:text-neutral-300">
                <div className="hidden sm:block flex flex-col md:flex md:flex-row relative justify-between segments sm:flex-row items-center space-y-3 sm:space-y-0 text-center sm:text-left sm:space-x-3 ">
                  {col.options[1] && renderMultiAirlines(col, col.options[1].flightSegment, 'd')}
                </div>
                <div className="flex flex-col sm:flex-row items-end space-y-3 sm:space-y-0 sm:text-right sm:space-x-3 ">
                  <div>
                    <span className="block text-md font-medium">
                      {col.options[1] && getAirportCity(col.options[1].flightSegment[0].depature)}({col.options[1] && col.options[1].flightSegment[0].depature})
                    </span>
                    <span className="block text-lg font-medium">
                      {col.options[1] && moment(col.options[1].flightSegment[0].depTime).format('HH:mm')}
                    </span>
                    <span className="mt-1.5 text-xs text-neutral-500 dark:text-neutral-400">
                      {col.options[1] && moment(col.options[1].flightSegment[0].depTime).format('ll')}
                    </span>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 text-center sm:text-center sm:space-x-3 ">
                  <div>
                    <span className="block text-sm text-neutral-500 dark:text-neutral-400">
                      {col.options[1] && getDuration(col.options[1].duration)}
                    </span>
                    <div className="w-20 border-b mt-2 mb-2 border-neutral-300 dark:border-neutral-700"></div>
                    <span className="block text-sm text-neutral-500 text-primary-700 dark:text-neutral-400">
                      {(col.options[1] && col.options[1].flightSegment.length > 2) ? `${col.options[1].flightSegment.length - 1} Stops` : (col.options[1] && col.options[1].flightSegment.length > 1) ? `${col.options[1].flightSegment.length - 1} Stop` : `Non Stop`}
                    </span>
                    <span className="block text-xs text-neutral-500 dark:text-neutral-400">
                      {col.options[1] && renderLayover(col.options[1].flightSegment)}
                    </span>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 text-center sm:text-left sm:space-x-3 ">
                  <div>
                    ...
                    <span className="block text-xl font-medium">
                      {col.options[1] && moment(col.options[1].flightSegment[col.options[1].flightSegment.length - 1].arrTime).format('HH:mm')}
                    </span>
                    <span className="mt-1.5 w-20 text-xs text-neutral-500 dark:text-neutral-400">
                      {col.options[1] && moment(col.options[1].flightSegment[col.options[1].flightSegment.length - 1].arrTime).format('ll')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap sm:flex-row items-center justify-between xl:justify-start xl:space-x-8 text-sm text-neutral-700 dark:text-neutral-300">
                ...
                {(col.options[1] && col.options[1].flightSegment && col.options[1].flightSegment[0].seat && col.options[1].flightSegment[0].seat !== "") &&
                  <Badge className='items-center text-xs' name={<><img src={FlightSeat} className='w-3 h-3' />{`${col.options[1].flightSegment[0].seat.Number} Seat(s) Left`}</>} />
                }
              ...
              <div className="inline-block py-8 h-screen w-full">
                <div className="inline-flex flex-col w-full max-w-4xl text-left align-middle transition-all transform overflow-hidden rounded-2xl bg-white dark:bg-neutral-900 dark:border dark:border-neutral-700 dark:text-neutral-100 shadow-xl h-full">
                  ...
                  <div className="px-8 mt-2 flex-grow overflow-y-auto text-neutral-700 dark:text-neutral-300 divide-y divide-neutral-200">
                    <CardView tripType={'return'} col={cardData} />
                  </div>
