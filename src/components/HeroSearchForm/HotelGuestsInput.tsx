...
const GuestsInput: FC<GuestsInputProps> = ({
  ...
  const [guestAdultsInputValue, setGuestAdultsInputValue] = useState(
    defaultValue.guestAdults || 2
  );
  const [guestChildrenInputValue, setGuestChildrenInputValue] = useState(
    defaultValue.guestChildren || 0
  );
  const [guestRoomInputValue, setGuestRoomsInputValue] = useState(
    defaultValue.guestRooms || 1
  );
  const [guestClassValue, setGuestClassValue] = useState(
    null
  );

  useEffect(() => {
    setGuestAdultsInputValue(defaultValue.guestAdults || 2);
  ...
  useEffect(() => {
    if (onChange) {
      onChange({
        ...
        guestRooms: guestRoomInputValue,
      });
    }
  }, [guestAdultsInputValue, guestChildrenInputValue, guestRoomInputValue]);

  const onChangeClass = (e :any) => {
    setGuestClassValue(e.target.value)
  }
        ...
        <>
          ...
          >
            ...
            <div className="flex-grow">
              <span className="block xl:text-lg font-semibold">
                 {guestRoomInputValue || ""} Rooms & {totalGuests || ""} Guests
              </span>
              <span className="block mt-1 text-sm text-neutral-400 leading-none font-light">
                {guestClassValue ? guestClassValue : "Guests & Rooms"}
              </span>
              {!!totalGuests && open && (
                <ClearDataButton
                  onClick={() => {
          ...
          >
            <Popover.Panel className="absolute right-0 z-10 w-full sm:min-w-[340px] max-w-sm bg-white dark:bg-neutral-800 top-full mt-3 py-5 sm:py-6 px-4 sm:px-8 rounded-2xl shadow-xl">
             <NcInputNumber
                className="w-full"
                defaultValue={guestRoomInputValue}
